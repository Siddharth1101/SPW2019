...
public:
	...
	void print();
	void print(unsigned additional);
};
...
void TreeNode::print(unsigned additional) {
	std::cout << data << " (" << additional << ")" << std::endl;
}
class Tree {
	PtrToNode root;
public:
...
unsigned Tree::postorder(PtrToNode rr) {
	unsigned subtreesize = 1;
	if (rr) {
		for (auto child: rr->children) {
			subtreesize += postorder(child);
		}
		rr->print(subtreesize);
	}
	return subtreesize;
}
unsigned Tree::postorder() {
